...
const GameBoard = (() => {
	...
	const start = () => { //start game
	  ...
	  patternsO = []; //perbuatan object O
  	patternsX = []; //permbuatan objet x
		gameBoxes.forEach((box) => {
  	  box.classList.remove('O', 'X', 'selected'); //hasil
	  });

		// set message "Player 1's Turn"
		gameInfo.style.display = 'inline';
		gameInfo.innerText = `${gamePiece[0]}'s Turn`; //untuk memunculan object ke 0 = X
		console.log("start game")
	}

	function addMark(e) { // dalam adalah hasil pengantian pemain secara off line atau dianggap 2player
...
const GameController = (() => {
	...
	function getCombinations(chars) {
	  ...
	  let f = function(prefix, chars) {
	    for (let i = 0; i < chars.length; i++) { //looping untuk panjang 3 maks
	      result.push(prefix + chars[i]);
	      f(prefix + chars[i], chars.slice(i + 1));
	    }
	  }
	  f('', chars);
	  filteredResult = result.filter(e => e.length === 3);// maksimal 3
	  return filteredResult;
	}

	function gameOver(tie) {
		if (tie === false) {
			gameInfo.innerText = `${gamePiece[1]} is the winner!`;// bisa dibilang untuk memunculkan kalimat dalam `${gamePiece[1]} is the winner!`
			replay(); // melakukan replay
		} else { //jika terjadi hal lain akan melanjutkan else yang di bawah
			gameInfo.innerText = 'Tie game!'; //ini untuk memunculkan  kalimat 'tie game'
			replay();
		}
	}
	...
	return { findMatch, getCombinations, gameOver, replay } // return untuk hasil awalan lagi dengan hasil sekarang
